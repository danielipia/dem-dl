...
def solveLBFGS(t):
    """
    Solve via L-BFGS with barrier loss:

        argmin_{x>=0}  mu * sum_i log(1 + exp(...)) + alpha_l1 * ||x||_1 + 0.5 * alpha_l2 * ||x||^2

    subject to lb <= Dx <= ub, x >= 0  (enforced via softplus: x = softplus(u))

    If args.lbfgs_normalize=True, rows of D and meas are divided by
        tol = max(meas - lb, _minimumDNs)
    before building the loss, making the fit term chi-square-like (same
    rescaling used in solveElasticNet / solveLPRefit).
    """
    import torch
    ...
    D_np, meas, lb_np, ub_np, args = t

    # ------------------------------------------------------------------ #
    # Optional noise-normalisation (chi-square rescaling)                  #
    # ------------------------------------------------------------------ #
    normalize = getattr(args, 'lbfgs_normalize', False)
    if normalize:
        tol = np.maximum(meas - lb_np, _minimumDNs)   # shape (n_obs,)
        D_np  = D_np  / tol[:, None]                  # (n_obs, n_basis)
        meas  = meas  / tol
        lb_np = lb_np / tol
        ub_np = ub_np / tol
    ...
    # Build loss                                                           #
    # ------------------------------------------------------------------ #
    barrier_args = SimpleNamespace(
        alpha_l1  = getattr(args, 'lbfgs_l1',    1.0),
        alpha_l2  = getattr(args, 'lbfgs_l2',    0.0),
        mu        = getattr(args, 'lbfgs_mu',  1000.0),
        alpha_fit = getattr(args, 'lbfgs_fit',    0.0),
    )

    D_t    = torch.tensor(D_np, dtype=torch.float64)
    R_t    = D_t                                              # placeholder
    B_t    = torch.eye(D_np.shape[1], dtype=torch.float64)   # idem

    loss_fn = BarrierLoss(D=D_t, R=R_t, B=B_t, args=barrier_args)

    lb_t   = torch.tensor(lb_np, dtype=torch.float64).unsqueeze(0)
    ub_t   = torch.tensor(ub_np, dtype=torch.float64).unsqueeze(0)
    meas_t = torch.tensor(meas,  dtype=torch.float64).unsqueeze(0)

    # ------------------------------------------------------------------ #
    # Optimise                                                             #
    # ------------------------------------------------------------------ #
    u = torch.ones(1, D_np.shape[1], dtype=torch.float64, requires_grad=True)

    lbfgs = torch.optim.LBFGS(
        [u],
        lr=0.1,
        max_iter=getattr(args, 'lbfgs_steps', 5000),
    ...
    def closure():
        lbfgs.zero_grad()
        x = nn.functional.softplus(u)
        loss = loss_fn(x, aia_obs=meas_t, lb=lb_t, ub=ub_t)
        loss.backward()
        return loss
    ...
    with torch.no_grad():
        x_basis = nn.functional.softplus(u).squeeze(0)

    return x_basis.numpy()
...
def parseArgs():
    ...
    # LBFGS arguments
    parser.add_argument("--lbfgs_normalize", default=False, action="store_true", help="divide D and meas by noise sigma before L-BFGS (chi-square rescaling)")
    parser.add_argument("--lbfgs_l1",    default=1.0,    type=float, help="L1 regularization weight for solveLBFGS_torch (promotes sparsity in the DEM)")
    parser.add_argument("--lbfgs_mu",    default=1000.0, type=float, help="Barrier penalty strength for solveLBFGS_torch (enforces lb <= Dx <= ub; higher = stricter constraints)")
    parser.add_argument("--lbfgs_l2",    default=0.0,    type=float, help="L2 regularization weight for solveLBFGS_torch (smooths the DEM; 0 = disabled)")
    parser.add_argument("--lbfgs_fit",   default=0.0,    type=float, help="Fit term weight for solveLBFGS_torch (chi-square goodness of fit; 0 = constraints only)")
    parser.add_argument("--lbfgs_steps", default=5000,    type=int,   help="Max L-BFGS iterations per pixel for solveLBFGS_torch")
    # Subsample arugments
